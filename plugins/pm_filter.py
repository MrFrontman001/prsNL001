...
async def next_page(bot, query):
    ...
    if settings['links']:
        ...
        for file_num, file in enumerate(files, start=offset+1):
            files_link += f"""<b>\n\n{m📕 <a href=https://t.me/{temp.U_NAME}?start=file_{query.message.chat.id}_{file['_id']}>[{get_size(file['file_size'])}] {file['file_name']}</a></b>"""
    else:
        btn = [[
    ...
    if n_offset == 0:
        btn.append(
            [InlineKeyboardButton("« ʙᴀᴄᴋ", callback_data=f"next_{req}_{key}_{off_set}"),
             InlineKeyboardButton(f"🗓️ {math.ceil(int(offset) / MAX_BTN) + 1}/{math.ceil(total / MAX_BTN)}", callback_data="buttons")]
        )
...
async def filter_languages_cb_handler(client: Client, query: CallbackQuery):
    ...
    if settings['links']:
        ...
        for file_num, file in enumerate(files, start=1):
            files_link += f"""<b>\n\n📕 <a href=https://t.me/{temp.U_NAME}?start=file_{query.message.chat.id}_{file['_id']}>[{get_size(file['file_size'])}] {file['file_name']}</a></b>"""
    else:
        btn = [[
            InlineKeyboardButton(text=f"{get_size(file['file_size'])} - {file['file_name']}", callback_data=f"file#{file['_id']}")
        ]
    ...
    if n_offset == 0:
        btn.append(
            [InlineKeyboardButton("« ʙᴀᴄᴋ", callback_data=f"lang_next#{req}#{key}#{lang}#{b_offset}#{offset}"),
             InlineKeyboardButton(f"🗓️ {math.ceil(int(l_offset) / MAX_BTN) + 1}/{math.ceil(total / MAX_BTN)}", callback_data="buttons")]
        )
...
async def quality_search(client: Client, query: CallbackQuery):
    ...
    if l_offset != "":
        btn.append(
            [InlineKeyboardButton(text=f"🗓️ 1/{math.ceil(int(total_results) / MAX_BTN)}", callback_data="buttons"),
...
async def auto_filter(client, msg, s, spoll=False):
    ...
    if settings['links']:
        btn = []
        for file_num, file in enumerate(files, start=1):
            files_link += f"""<b>\n\n📕 <a href=https://t.me/{temp.U_NAME}?start=file_{message.chat.id}_{file['_id']}>[{get_size(file['file_size'])}] {file['file_name']}</a></b>"""
    else:
        btn = [[
            InlineKeyboardButton(text=f"{get_size(file['file_size'])} - {file['file_name']}", callback_data=f'file#{file["_id"]}')
        ]
...
async def advantage_spell_chok(message, s):
    ...
    btn = [[
        InlineKeyboardButton("📑 ɪɴsᴛʀᴜᴄᴛɪᴏɴs", callback_data='instructions'),
        InlineKeyboardButton("ɢᴏᴏɢʟᴇ 🔍", url=f"https://www.google.com/search?q={google_search}")
    ]]
    try:
